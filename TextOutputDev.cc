...
void TextPage::dump(void *outputStream, TextOutputFunc outputFunc,
		    GBool physLayout, char *separator) {
  UnicodeMap *uMap;
  TextFlow *flow;
  TextBlock *blk;
  TextLine *line;
  ...
  // output the page, maintaining the original physical layout
    ...
    /*for (i = 0; i < nFrags; ++i) {
      ...
      int cnt = frag->col;
      char buff[255];
      int num = sprintf(buff,"(%d),",cnt);
      (*outputFunc)(outputStream, buff,num);
      }*/
    ...
    for (i = 0; i < nFrags; ++i) {
      ...
      // column alignment
      //printf("STUFFER : %d",frag->col);
      int cnt = frag->col;

      if(separator != NULL && cnt > 1) {
	(*outputFunc)(outputStream, separator, strlen(separator));
      }
	...
	//  (*outputFunc)(outputStream, "-", 1);
      ...
      // print the line
	     ...
	     //char buff[25];
	   //int x = sprintf(buff,"(%d)",cnt);
	   //strcat(big,buff);
	   //(*outputFunc)(outputStream, buff, x);
...
void TextOutputDev::endPage() {
  ...
  if (outputStream) {
    text->dump(outputStream, outputFunc, physLayout,separator);
  }
}
