...
void GlobalParams::parseLine(char *buf, GString *fileName, int line) {
  ...
  if (tokens->getLength() > 0 &&
    ...
    } else if (!cmd->cmp("textSeparator")) {
      parseTextSeparator(tokens, fileName, line);
...
void GlobalParams::parseTextSeparator(GList *tokens, GString *fileName,
				      int line) {
  if (tokens->getLength() != 2) {
    error(errConfig, -1,
	  "Bad 'textSeparator' config file command ({0:s}:{1:d})",
	  fileName, line);
    return;
  }
  delete textSeparator;
  textSeparator = ((GString *)tokens->get(1))->copy();
}
...
GString *GlobalParams::getTextSeparatorName() {
  GString *s;

  lockGlobalParams;
  s = textSeparator->copy();
  unlockGlobalParams;
  return s;
}
...
void GlobalParams::setSeparator(char *separatorName) {
  lockGlobalParams;
  delete textSeparator;
  textSeparator = new GString(separatorName);
  unlockGlobalParams;
}
